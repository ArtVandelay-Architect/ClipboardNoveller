...
function MovePageUp (ThisFile, PageNum) {
	...
	let nonsense = p1 + "_oreetdoloremagnaalisnostcit";
	// Swap files
	fm.move (p1, nonsense);
	fm.move (p2, newP1);
...
async function OpenPage (ThisFile, index) {
	...
	if (GetFileSuffix (PagePath) == "txt") {
		...
		EditPage (ThisFile, index, resultString);
...
// Continuing our fine tradition of not making permanent
// damage, it simply inserts a page after the previous page
// and push all the rest of pages backwards
async function EditPage (ThisFile, index, resultString) {
	let files = LoadFiles ();

}

async function AddPageBase(ThisFile, index, filetype, content) {
	// Linear search to update the json file
	for (c = 0; c < files.length; c++){
		file = files[c];
		if (file.filename == ThisFile.filename) {
